...
export function Session() {
  ...
  createEffect(() => {
    ...
    // Render ALL logo rows (autostock is a 2-line wordmark = 8 rows; the old code hardcoded
    // logo[0..3] and clipped "stock"). F5 BR-14.
    const logoLines = UI.logo("  ").split(/\r?\n/)
    return exit.message.set(
      [
        ...logoLines,
        ``,
        `  ${weak("Session")}${UI.Style.TEXT_NORMAL_BOLD}${title}${UI.Style.TEXT_NORMAL}`,
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}autostock -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
